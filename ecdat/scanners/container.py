...
from ecdat.domain.models import RawFinding, SecurityFinding
...
class ContainerScanner(Scanner):
    ...
    def scan(self, ctx: ScanContext) -> Iterator[RawFinding | SecurityFinding]:
        ...
        for tgt in targets:
            ...
            if rel in (".", ""):
                rel = tgt.name
            with tempfile.TemporaryDirectory(prefix="ecdat-img-") as tmp:
...
def _unpack(image: Path, merged: Path) -> list[str]:
    ...
    digests: list[str] = []
    with tempfile.TemporaryDirectory(prefix="ecdat-layers-") as ld:
        layers_dir = Path(ld)
        try:
